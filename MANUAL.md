# スマート案内図エディタ 公式操作マニュアル (v0.1.12)

プロレベルの不動産案内図・建築確認等の各種申請見取図が、**実地図をなぞるだけで直感的に2〜3分で完成！**  
...
### 3. 【新機能】各種申請（付近見取図）作図モード（1/2500 国土地理院地図）
ヘッダーの **「📑 各種申請」** タブを押すだけで、建築確認申請や開発許可申請の添付図面（建築基準法施行規則第1条の3準拠）を即座に作成可能！案内図作図モードとは作図データが完全に分離されているため、相互に干渉しません。
- **A4/B5定型用紙 & 四方10mm余白:** A4（横/縦）・B5（横/縦）を選択可能。法令基準の前後左右10mm余白枠線を自動生成。
- **1/2,500 法定標準縮尺 & 方位記号:** 内寸作図枠基準で 1mm = 2.5m を自動計算。50m / 100m / 200m の測定スケールバーと方位記号を自動配置。
- **表題欄（建築士情報完備）:** 申請地 地名地番、住居表示、**建築士番号、氏名**、縮尺を美麗な申請用ブロックで自動描画（四隅どこへでも配置可能）。文字サイズも大きく見やすく配置。
- **📐 区画割ツール（敷地特定）:** 街中の「角から何件目」を特定するための敷地ポリゴン作成機能。クリックで区画を描画し、ダブルクリックで「この区画を申請地に指定する」をワンタッチ切り替え（赤枠・赤着色ハイライト）。
- **申請地記号（◎）と文字の完全独立移動:** 二重丸記号と名称テキストプレートをそれぞれ別々にドラッグして最適配置可能。
...
- **現在のバージョン**: **`v0.1.12`** (`frontend/js/version.js` および `js/version.js` にて一元管理)
- **厳守ルール**:
